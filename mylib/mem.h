...
char* memory:: getfreebytes(int size){
	...
	last_aloc = ((lsp - bitmap) << 3); 
	return (shared_memory + ((lsp - bitmap) << 3));
}
char* memory:: store(char *ptr){
	return store(ptr, strlen(ptr));
}
char* memory:: store(char *ptr, int size){
	if(!page_size)
		return NULL;
	char *lsp, *rptr;
...
int memory:: del(char *ptr){
	return del(ptr , strlen(ptr));
}
